...
Axe::Axe(const Vector& size, SpriteAnimation* spriteAnim, const std::string& description,
         ...
         float atkSpeedMultiplier, int level, float projSpeed, float projLifetime, float cooldown)
    : Weapon(size, spriteAnim, description, flatDamage, flatAtkSpeed, damageMultiplier, atkSpeedMultiplier, level, cooldown),
...
Axe::Axe(const Vector& size,
    ...
    : Axe(size, spriteAnimation, description, 100.0f, 10.0f, 10.0f, 10.0f, 1, 150.0f, 10.0f, 3.0f)   
{
}
...
void Axe::attack(const Vector& position, const Vector& direction, Player* owner) {
    ...
    auto p = std::make_unique<Attack>(
        ...
        50.0f,
        std::move(anim),
        std::make_unique<WaveMotion>(direction, 100.0f, 10.0f, 5.0f),
        std::make_unique<TimedLifetime>(30.0f),
        false
    );
