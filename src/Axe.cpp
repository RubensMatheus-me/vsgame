...
Axe::Axe(const Vector& size, SpriteAnimation* spriteAnim, const std::string& description,
    ...
    : RangedWeapon(size, spriteAnim, description, flatDamage, flatAtkSpeed, damageMultiplier, atkSpeedMultiplier, level),
      axeProjectileAnimation(axeProjectileAnim),
      projectileSpeed(projSpeed),
      projectileLifetime(projLifetime)
{}

void Axe::attack(const Vector& position, const Vector& direction, Entity* owner) {
    ...
    getProjectiles().push_back(std::move(p));
}
